...
//now this request package will make the request to the google url server and 
//recieve the response and callback func will work and will provide the error , body to the down goven func 
//we use callback func to prevent it from waiting
...
function callback(error, response, html) {
    ...
    if (!error) {
       ...
       // loop written for extracting the tr's which are basically the rows of the table beiong selected 
        for (let i = 0; i < table.length; i++) {
            ...
            for(let j = 0; j<tbleRws.length; j++){
                ...
                data[manipulationTool(rowsColm[1]).text().trim()] = "rating", manipulationTool(rowsColm[2]).text().trim();
